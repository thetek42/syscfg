...
#define COLOR(u32)              {((u32 >> 24) & 0xFF) / 255.0f, \
                                 ((u32 >> 16) & 0xFF) / 255.0f, \
                                 ((u32 >>  8) & 0xFF) / 255.0f, \
                                 ((u32 >>  0) & 0xFF) / 255.0f} \
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		for (b = buttons; b->func != NULL; b++) {
			if (CLEANMASK(mods) == CLEANMASK(b->mod) && event->button == b->button && click == b->click) {
...
createmon(struct wl_listener *listener, void *data)
{
	...
	for (r = monrules; ; r++) {
		if (!r->name || strstr(wlr_output->name, r->name)) {
			...
			m->nmaster = r->nmaster;
			m->lt[0] = r->lt;
			m->lt[1] = &layouts[layout_count > 1 && r->lt != &layouts[1]];
			strncpy(m->ltsymbol, m->lt[m->sellt]->symbol, sizeof(m->ltsymbol));
	...
	m->fullscreen_bg = wlr_scene_rect_create(layers[LyrFS], 0, 0, (float[4])COLOR(fullscreen_bg));
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (cursor_mode == CurMove) {
		...
		box.x      = grabc->geom.x      + (rzcorner & 1 ? 0   :  cdx);
		box.y      = grabc->geom.y      + (rzcorner & 2 ? 0   :  cdy);
		box.width  = grabc->geom.width  + (rzcorner & 1 ? cdx : -cdx);
		box.height = grabc->geom.height + (rzcorner & 2 ? cdy : -cdy);
		resize(grabc, box, 1);
...
{
	static const char *cursors[] = { "nw-resize", "ne-resize", "sw-resize", "se-resize" };
...
setup(void)
{
	...
	root_bg = wlr_scene_rect_create(&scene->tree, 0, 0, (float[4])COLOR(rootcolor));
...
updatebar(Monitor *m)
{
	...
	if (!(drwl_font_create(m->drw, barfont, fontattrs)))
